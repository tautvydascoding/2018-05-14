...
//paciento paemimas is DB
function getPatient ($pacientoNr) {
    if (!is_numeric($pacientoNr)) {
        return NULL;//jei ivestas ne skiacius grazina NULL ir nutruakia funkcija
    }
    $manoSql = "SELECT * FROM patients WHERE id='$pacientoNr'; ";//'$nr' su viengubomis kabutemis veikaia kingtamasis
    $rezultatai = mysqli_query(getPrisijungimas(), $manoSql);//ivykdo sql koda arba kalba, grizta NYSQL OBJEKTAS
    ...
    if (mysqli_num_rows($rezultatai) > 0) {
        ...
        echo "ERROR!! paciento nr:  $pacientoNr neradome. Bandykite kita <br>"
            .mysqli_error(getPrisijungimas());
        return NULL;
    }
}
// dar karta ti ktris tas funkcijas
// function getImg ($imgId) {
//     $mySql = "SELECT * FROM img WHERE id='$imgId';";
//     $rezultatai = mysqli_query(getPrisijungimas(), $mySql);
// }
